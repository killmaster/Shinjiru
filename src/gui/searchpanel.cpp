...
SearchPanel::SearchPanel(QWidget *parent) :
  QDialog(parent), ui(new Ui::SearchPanel) {
  qDebug() << "Launching search panel";
  ui->setupUi(this);
  setAttribute(Qt::WA_DeleteOnClose);
  setWindowFlags(windowFlags() & ~Qt::WindowContextHelpButtonHint);

  connect(ui->searchButton, &QPushButton::clicked, [&]() {  // NOLINT
    QUrl search_url =
        API::sharedAPI()->sharedAniListAPI()->API_ANIME_SEARCH(
          ui->lineEdit->text());

    QJsonArray results =
        API::sharedAPI()->sharedAniListAPI()->get(search_url).array();
  ...
  connect(ui->webView, &QWebView::linkClicked, [&](const QUrl &url) {  // NOLINT
    ...
    connect(ap, &AnimePanel::accepted, this, [&, old_status, a]() {  // NOLINT
